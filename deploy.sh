...
SLIDES_PRESENTATION_ID=""                # (ไม่บังคับ) id ของ Google Slides deck — ว่าง = ข้ามการอัปเดตสไลด์
CYCLE_TABLE="user_token_cycle"           # B2C: repeat analysis (user × cycle)
REPEAT_TABLE="user_repeat_behavior"      # B2C: repeat analysis (user)
...
gcloud run jobs deploy "$JOB_NAME" \
  ...
  --set-env-vars="^@^GCP_PROJECT_ID=${PROJECT_ID}@BQ_LOCATION=${REGION}@BQ_DATASET=${DATASET_B2C}@BQ_DATASET_B2B=${DATASET_B2B}@BQ_DATASET_TOTAL=${DATASET_TOTAL}@BQ_TABLE=${TABLE}@BQ_STATE_TABLE=${STATE_TABLE}@BQ_TOTAL_VIEW=${TOTAL_VIEW}@MONGO_DB=${MONGO_DB_NAME}@MONGO_COLLECTION=${TABLE}@PIPELINE_TIMEZONE=${PIPELINE_TZ}@START_DATE=${START_DATE}@LOOKBACK_DAYS=${LOOKBACK_DAYS}@EXCHANGE_RATE=${EXCHANGE_RATE}@ID_INDEX_BUFFER_HOURS=${ID_INDEX_BUFFER_HOURS}@MONGO_PACKAGE_COLLECTION=${PACKAGE_COLLECTION}@BQ_PACKAGE_TABLE=${PACKAGE_TABLE}@MONGO_USERS_DB=${USERS_DB}@MONGO_USERS_COLLECTION=${USERS_COLLECTION}@BQ_USERS_TABLE=${USERS_TABLE}@MONGO_COMPANY_COLLECTION=${COMPANY_COLLECTION}@MONGO_TEAM_COLLECTION=${TEAM_COLLECTION}@BQ_COMPANY_TABLE=${COMPANY_TABLE}@BQ_TEAM_TABLE=${TEAM_TABLE}@BQ_B2C_TABLE=${B2C_TABLE}@BQ_B2B_TABLE=${B2B_TABLE}@BQ_CYCLE_TABLE=${CYCLE_TABLE}@BQ_REPEAT_TABLE=${REPEAT_TABLE}@SLIDES_PRESENTATION_ID=${SLIDES_PRESENTATION_ID}@EXPECTED_EGRESS_IP=${EGRESS_IP}" \
